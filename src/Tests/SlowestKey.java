...
public class SlowestKey {
	...
	public static char slowestKey(int[] releaseTimes, String keysPressed) {
		...
		if (releaseTimes.length == 0 || keysPressed == null || keysPressed.length() != releaseTimes.length) {
			return ' ';
		}
		
		List<Character> slowest = new ArrayList<>();
		int slowInt = Integer.MIN_VALUE;
		
		for (int i = 0; i < releaseTimes.length; i++) {
			
			if (i == 0) {
				slowInt = releaseTimes[0];
				slowest.add(keysPressed.charAt(0));
			} else {
				...
				if (time > slowInt) {
					slowInt = time;
					slowest.clear();
					slowest.add(keysPressed.charAt(i));
				} else if (time == slowInt) {
					slowest.add(keysPressed.charAt(i));
				}
			}
		}
		...
		if (slowest.size() == 1) {
			return slowest.get(0);
		}
		
		Collections.sort(slowest);
		
		return slowest.get(slowest.size() - 1);
    }
}